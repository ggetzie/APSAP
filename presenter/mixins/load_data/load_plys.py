...
class LoadPlys:
    ...
    def populate_models(self):
        ...
        This function populates the list that not sorted by similarity but by year,
        batch then piece number
        """
        ...
        for batch_year in sorted(years_models.keys()):
            ...
            for batch in sorted(years_models[batch_year].keys()):
                ...
                for batch_piece in sorted(years_models[batch_year][batch].keys()):
                    ...
                    # We measure the 3d model in the path to pre-cache the value
                    main_presenter.measure_pixels_3d(path)
                    # We set the modelPiece item that will be appended to the modelPiece item
                    model_piece = QStandardItem(f"{batch_piece}")
                    # Remember to store the path in the item
                    model_piece.setData(f"{path}", Qt.UserRole)
    ...
    def get_year_models(self):
        ...
        Returns:
            dict: A dictionary of years as keys. Dictionaries are nested in dictionaries.
            You can see them as trees.
        """
        ...
        if not all_model_paths:
            main_view.statusLabel.setText("No models were found")
        ...
        for path in all_model_paths:
            ...
            if year is None or batch is None or piece is None:
                continue
            ...
            # This is a dictionary with 2022 as a key, pointing to another dictionary with
            # 001 as the key, pointing to another dictionary with 03 as the key and so on.
            # Now if we have 2022, 001, 02, we merely add 02 to the dictionary pointed by 001
            if not year in models_dict:
                models_dict[year] = {}
            if not batch in models_dict[year]:
                models_dict[year][batch] = {}
