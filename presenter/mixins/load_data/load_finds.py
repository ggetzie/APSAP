...
class LoadFinds:
    ...
    def populate_finds(self):
        ...
        main_view.dict_find_2_ply = {}
        main_view.dict_ply_2_find = {}
        ...
        for find in finds:
            ...
            logging.info("Loading the folder: %s", find_base_path)
            ...
            if not (
                Path(find_base_path / "1.jpg").is_file()
                and Path(find_base_path / "2.jpg").is_file()
            ):
                continue

            # Check if the find number is within the filter range
            if int(find) < int(main_view.find_start.value()) or int(find) > int(
                main_view.find_end.value()
            ):
                continue
            ...
            (batch_year, batch_num, batch_piece) = main_model.get_sherd_info(
                easting, northing, context, find
            )
            if (
                batch_year is not None
                and batch_num is not None
                and batch_piece is not None
            ):
        ...
        logging.info("Showing possible duplicates")
        for key, value in duplicates_checks.items():
            if len(value) >= 2:
