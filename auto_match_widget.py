...
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QVBoxLayout, QPushButton, QScrollArea, QGroupBox, QFormLayout, QProgressBar,QLabel,QMessageBox
...
class auto_match_widget(QWidget):
    ...
    def update_auto_match_area(self):
        ...
        for sorted_target in self.sorted_target_list:
            ...
            new_candidate_card = candidate_card(self.main_widget, sorted_target[-2],sorted_target[-1],sorted_target[2],sorted_target[1],sorted_target[0])
    ...
    def message(self, error_batches):
        QMessageBox.information(self, "Warning", "3d models not finished, check the folder for: " + error_batches + ' ')
...
class Runnable(QRunnable):
    ...
    def activate_model(self):
        ...
        for one_batch in self.main_widget.suggested_batches:
            model_folder_path = Path(self.main_widget.cur_context_3dbatch_path).joinpath(one_batch, "registration_reso1_maskthres242", "final_output")
        ...
        for one_batch in self.main_widget.suggested_batches:
            ...
            if(not twod_projection_folder.exists()):
                process_one_batch(Path(self.main_widget.cur_context_3dbatch_path).joinpath(one_batch))
            else:
                continue
    
        if proceed:
            all_targets_for_context_list = read_context_targets(self.main_widget) #Path(self.curr_context_path).parents[1]
            ...
            for one_target in all_targets_for_context_list:
                ...
                i = i+1
                self.notifyProgress.progress.emit(int(100 * i/len(all_targets_for_context_list)))
...
class candidate_card(QWidget):
    def __init__(self,main_widget, front_img_path,back_img_path,batch_number,piece_number,match_point_number):
        QWidget.__init__(self)

        self.main_widget = main_widget
        self.batch_number = batch_number
        self.piece_number = piece_number
        self.batch_image_path = str(front_img_path.parents[2]) + "\\00_corres_img_pieces.png"
        self.model_path = str(front_img_path.parents[2]) + "\\piece_{}_world.ply".format(piece_number)

        self.candidate_img_frt = QLabel(self)
        self.candidate_img_frt.setPixmap(QPixmap.fromImage(QImage(str(front_img_path))).scaledToHeight(220))

        self.candidate_img_back = QLabel(self)
        self.candidate_img_back.setPixmap(QPixmap.fromImage(QImage(str(back_img_path))).scaledToHeight(220))
