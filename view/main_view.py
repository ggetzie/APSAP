...
class MainView(QMainWindow, PlyWindowMixin, OpenImageMixin):
    ...
    Args:
        QMainWindow (QMainWindow): Making thie MainView a QWidget that can load a ui file and display things.
        PlyWindowMixin (class): Mixin that initializes the window that displays 3d models in it.
        OpenImageMixin (class): Mixin that allows the users to click on an Image to see it full size.
    """    
    def __init__(self):
        """This constructor loads the ply to the ui file, set up the 3d model window, and make it the images pop
    ...
    def set_up_view_presenter_connection(self, main_presenter):
        """This function links the interaction from the user with the interface(main_view) via their handlers(main_presenter)

        Args:
            main_presenter (class): Containg all the functions that handle the interactions between the GUI and the Data

        """        
        main_view = self
    
        #Connecting the selects of hemisphere, zone, easting, northing and context with their handlers
