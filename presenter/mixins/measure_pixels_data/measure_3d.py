...
class Measure3dMixin:  # bridging the view(gui) and the model(data)
    ...
    def get_area_width_length_contour3d(self, path_3d):
        ...
        #Get the window to which we put the loaded model to it so we can take picture and measure the pixels
        ...
        #Get the width and length of the bounding box, which is the width and length of the 3d model
        ...
        #Get the area and countour of the 3d model.
    ...
    def get_width_length_3d(self, bounding_box):
        ...
        Returns:
            (width, length): The width and length of the bounding box, width is the shorter side, length is the longer side.
        """        
    ...
    def get_mm_pixels_ratio(self, bounding_box, ply_window):
        ...
        What it does is simple. It first creates a red bounding box. We then have the difference between two sides of the bounding box
        in mm, then we measure the difference between the two sides of the bounding box in pixels by taking a picture of it in the
        ply_window and analyze the pixels.   
        Because we know how far 1 mm is in real life and 1 pixel is in the picture. We can get their ratio

        Args:
            bounding_box (pillow image): The red bounding box indicates the pixel difference between the left 
        ...
        #The red pixels are the where the bounding box of the 3d model is, white pixels are the background
        #Here we get the center row of the picture from the y axis, so we have a list of white(for the most part) values and red values
        ...
        #We get the bounding box's pixels difference between the two side of the red bounding box,
        # So we can know the pixel differences between the left most and right most of the bounding box  
        
        if(len(red_box_red_locations)>=4):
            bounding_box_pixels_difference = (red_box_red_locations[2] - red_box_red_locations[1] )  
        else :
            bounding_box_pixels_difference = (red_box_red_locations[1] - red_box_red_locations[0] )  
    ...
    def get_area_3d(self, ply_window, mm_pixels_ratio):
        """This function gets the area of the ceremics in the ply window by counting the number of pixels
        from the picture taken in the ply_window and scale with the proper mm_pixels_ratio
        ...
        Returns:
            double: The area of the ceremics 
        """        
        ...
        #Counting the number of pixels where the pixels are not white, thus the number of pixels representing the sherd
    ...
    def get_contour_3d(self, ply_window):
        ...
        #Since white means background, black means ceremics for picture taken in ply_window, we have to flip the value
        #So that background is black, ceremic area is white.
        pic[pic==255] = 0
        pic[pic != 0] = 255
