...
def read_context_targets(main_widget):
    if main_widget.suggested_batches == []:
        ...
        context_front_list = [
            file
            for file in target_context_path.rglob("*")
            if file.name == "2d_image_front.png"
        ]
        context_back_list = [
            file
            for file in target_context_path.rglob("*")
            if file.name == "2d_image_back.png"
        ]
        ...
        if len(context_front_list) != len(context_back_list):
            ...
            for i in range(0, len(context_front_list)):
                master_context_targets_space.append(
                    (context_front_list[i], context_back_list[i])
                )
    else:
        context_front_list = []
        context_back_list = []
