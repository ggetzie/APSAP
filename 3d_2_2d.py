...
def process_one_batch(batch_folder):
    ...
    models_folder = batch_folder.joinpath('registration_reso1_maskthres242','final_output')
...
def process_one_context(trench_path, context_number):
    context_folder = pathlib.Path(trench_path).joinpath(str(context_number),'finds','3dbatch','2022')
    #double check folde exists 
    if not context_folder.exists():
        print("Batch has not finished building all the 3d models")
    else:
        batch_list = [batch for batch in context_folder.iterdir() if (batch.is_dir() and ('batch' in batch.stem) and ('einscan' not in batch.stem)) ] #read in all the files 
