...
class LoadImagesModels:
    ...
    def get_similaritiy_scores(self, index,  image_path):
        ...
        color_brightness_2d_image_2 = get_image_summary_from_2d(_2d_image_path_image_2)
        color_brightness_2d_image_1 = get_image_summary_from_2d(_2d_image_path_image_1)
        #Trying get the _2d_area here okay
        similarity_scores = []
        ...
        for i in range(len(self.all_3d_areas)):
            for j in range(len(self.all_3d_areas[i])):
                ...
                color_brightness_3d = (self.all_3d_color_summaries[i][j][0][3]) 
                ...
                brightness_similarity = min( max((color_brightness_2d_image_2[3]/1.2)/_2d_area_image_2, _2d_area_image_2/(color_brightness_2d_image_2[3]/1.2)) , max(color_brightness_3d/(color_brightness_2d_image_1[3]/1.2), (color_brightness_2d_image_1[3]/1.2)/color_brightness_3d))
                
                similarity_scores.append([area_similarity *(2.5/4) + brightness_similarity * (1.5/4), batch_num, piece_num])
