...
def identify_range_of_batches(trench_northing, trench_easting, current_context, current_piece):
    ...
    path = basedir.joinpath('data' '\{}_{}_reference.csv'.format(trench_northing,trench_easting))
    print(path)
    df = pd.read_csv(path)
    ...
    if (current_row_number == len(context_lines)-1):
        ...
        context_next = context_lines.loc[context_lines['row_num']==current_row_number+1]
        current_start_num = context_start.index[0]
        current_end_num = context_next.index[0]

    current_context_df = df.iloc[current_start_num:current_end_num,:]
    current_context_df['range'] = current_context_df['range'].dropna().apply(lambda x: int(x.split('-')[0]))

    batches_start = current_context_df[current_context_df['range']<=int(current_piece)].iloc[-1]
    ...
    if(current_batch_row_num == len(range_markers)-1):
        ...
        next_range = (range_markers.loc[range_markers['row_num']==current_batch_row_num+1]).index[0]
