...
from PyQt5.QtWidgets import QWidget, QLabel, QApplication, QHBoxLayout, QVBoxLayout, QPushButton, QTabWidget, QGroupBox,QComboBox,QTextEdit
import os
...
class Main_Widget(QWidget):
    ...
    def initUI(self):       
        ...
        self.root_path = "D:\\ararat\\data\\files\\N\\38\\{}\\{}\\".format(self.curr_trench[0],self.curr_trench[1])
        self.total_context_list = [int(x) for x in os.listdir(self.root_path) if x.isdigit()]
        ...
        self.cur_context_individual_path = self.root_path + "{}\\finds\\individual\\".format(self.curr_context)
        self.cur_context_3dbatch_path = self.root_path + "{}\\finds\\3dbatch\\2022\\".format(self.curr_context)
        if not os.path.isdir(self.cur_context_individual_path):
            self.individual_folder_exists = False
        else:
            self.total_img_list = [int(x) for x in os.listdir(self.cur_context_individual_path) if x.isdigit()]
            self.total_img_list.sort()
            self.total_img_list =  [str(x) for x in self.total_img_list]
            self.total_img_num = len(self.total_img_list)
            self.path_curr_img = self.cur_context_individual_path + "{}\\photos".format(self.curr_img)
        ...
        self.button_context_last.clicked.connect((lambda x: self.change_context(self.curr_context-1)))

        self.button_context_next = QPushButton(self)
        self.button_context_next.setText('next context')
        self.button_context_next.clicked.connect((lambda x: self.change_context(self.curr_context+1)))
        ...
        self.button_sherd_last.clicked.connect((lambda x: self.change_sherd(self.curr_img-1)))

        self.button_sherd_next = QPushButton(self)
        self.button_sherd_next.setText('next sherd')
        self.button_sherd_next.clicked.connect((lambda x: self.change_sherd(self.curr_img+1)))
        ...
        self.suggested_batches = identify_range_of_batches(self.curr_trench[0],self.curr_trench[1],str(self.curr_context),str(self.curr_img))
    ...
    def update_batch_info(self):
        batch_num, sherd_num = get_pottery_sherd_info(self.curr_trench[0],self.curr_trench[1],self.curr_context,self.curr_img)
    ...
    def update_database(self):
        ...
        update_match_info(self.curr_trench[0],self.curr_trench[1], self.curr_context, self.curr_img, new_batch_num, new_sherd_num)
